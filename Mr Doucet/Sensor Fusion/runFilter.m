function [CQ,AI,covX,covY] = runFilter(filter,sigma_P2,randomness,aniso)

particlesNumber = 50^2;             % A square number is better
noRandom = ~randomness;                    % True to enable reproductibility
anisotropic_case = aniso;           % Select isotropic/anisotropic case
    ...
    %   Geolocation process parameters
        ...
        switch filter
            ...
            %   EKF (UKF) initialisation:
                ...
                P_cov_ini=sigma_P2*eye(2);                             %   Initial state covariance guess - Change if needed 
        end
...
for k=1:N_loops_fb                                                          %   Begin main 'for' loop: for all time steps
    ...
    P_r_meas(k,1)=max(0,P_r_true(k,1)+sig_P_r_W*randn(1));                                                 %   Add noise in W
...
AI = norm(x_state(:,end)-x_t_vec');
switch filter
    case 'PF'
        covX = var(particles(1,:));
        covY = var(particles(2,:));
    otherwise
        covX = P_cov(1,1,end);
        covY = P_cov(2,2,end);
end
