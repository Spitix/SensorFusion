...
%   Area parameters and number of iterations are global variables
    ...
    %   Jammer parameters
   	...
   	%   inside the search area. These parameters are not known by the UAV
        ...
        switch noRandom
            case true
                P_t=(P_t_min+(P_t_max-P_t_min));
            case false
                P_t=(P_t_min+(P_t_max-P_t_min)*rand(1,1));
        end
...
%   Filters
    ...
    switch filter
        case 'PF'
            x_state=zeros(2,N_loops_fb);
            xi = zeros(100,N_loops_fb);
            yi = zeros(100,N_loops_fb);
            fx = zeros(100,N_loops_fb);
            fy = zeros(100,N_loops_fb);
        otherwise
...
for k=1:N_loops_fb                                                          %   Begin main 'for' loop: for all time steps
    ...
    if anisotropic_case
        ...
        [EV,AV] = meshgrid(elevvals', azimvals');
        G_t = interp2(EV,AV,gainvals,elev_angle(k,1)*r2d,azimuth_rel_angle(k,1)*r2d);
    ...
    %   Kalman filtering: EKF (UKF)
        ...
        if (((obs_check==1)&&(k==k_obs))||(re_run_bool==1))                 %   If intersections have begun & first time EKF (UKF) is run
            ...
            switch filter
                ...
                case 'UKFna'
                    [x_state(:,k),P_cov(:,:,k),K_EKF_gain(:,k)]=UKF_form_non_augmented(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,Q_KF,R_KF);
                case 'PF'
                    [x_state(:,k),particles,xi(:,k),fx(:,k),yi(:,k),fy(:,k)]=PF_form(particles,x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),R_KF);
                otherwise
        ...
        elseif ((obs_check==1)&&(k>k_obs))                                  %   If intersections have begun & EKF (UKF) has alreay started
            switch filter
                ...
                case 'UKFna'
                    [x_state(:,k),P_cov(:,:,k),K_EKF_gain(:,k)]=UKF_form_non_augmented(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state(:,k-1),P_cov(:,:,k-1),Q_KF,R_KF);
                case 'PF'
                    [x_state(:,k),particles,xi(:,k),fx(:,k),yi(:,k),fy(:,k)]=PF_form(particles,x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),R_KF);
                otherwise
                    error('Bad parameter for filter, see line 34');
    ...
    if strcmp(filter,'PF')
        plot_animation_search_PF(N_plots,k,x_t_vec,x_vec_all(1:k,:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:k),k_obs,N_loops_fb,p_e,0,psi_jammer,particles);
    else
...
for k=(N_loops_fb+1):N_loops_vf
    ...
    if ~strcmp(filter,'PF')
        ...
        plot_animation_search_PF(N_plots,k,x_t_vec,x_vec_all(1:k,:),psi_all(k,1),r_est_l(k,1),r_est_h(k,1),centre_geo_circle(k,:),radius_geo_circle(k,1),x_state(:,1:k),k_obs,N_loops_fb,p_e,r_d,psi_jammer,particles); 
    end
...
figure
plot(t,x_state(1,:),'.',t,x_state(2,:),'.',t,x_t_vec(1)*ones(1,size(x_state,2)),'--',t,x_t_vec(2)*ones(1,size(x_state,2)),'--');
legend('Xestimated','Yestimated','Xreal','Yreal');

% measure performance index
error = (x_state - repmat(x_t_vec',1,size(x_state,2))).^2;
dist = sqrt(error(1,:)+error(2,:));
index = sum(dist)/length(dist)
distance = norm(x_state(:,end)-x_t_vec')