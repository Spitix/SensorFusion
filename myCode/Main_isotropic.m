...
%         sig_P_r_W=P_thermal_noise/3;                                      %   std sigma based on thermal noise [W]
    ...
    %   Geolocation process parameters
        ...
        %   Parameters for EKF (UKF) start
            ...
            P_cov_ini=6000^2*eye(2);                                        %   Initial state covariance guess - Change if needed 
...
for k=1:N_loops_fb                                                             %   Begin main 'for' loop: for all time steps
    ...
    %   Kalman filtering: EKF (UKF)
        ...
        if (((obs_check==1)&&(k==k_obs))||(re_run_bool==1))              	%   If intersections have begun & first time EKF (UKF) is run
            ...
            R_KF = (sig_P_r_W^2)/P_thermal_noise;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %---------- < Design a EKF (UKF) > ------------%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
figure(2),clf
subplot(221),hold on
    plot(x_t_vec(1)*ones(N_loops_fb,1),'--r')
    plot(x_state(1,:),'k')
...
%     plot(x_state(1,:)-sigma_x',':b')
    hold off
subplot(222),hold on
    plot(x_t_vec(2)*ones(N_loops_fb,1),'--r')
    plot(x_state(2,:),'k')
...
figure(3),clf
subplot(211),hold on
    plot(x_t_vec(1)*ones(n_loop,1),'--r')
    plot(x_state(1,(N_loops_fb+1):N_loops_vf),'k')
%     sigma_x2=zeros(n_loop,1);
%     sigma_x2(:)=P_cov(1,1,(N_loops_fb+1):N_loops_vf);
%     sigma_x=sqrt(sigma_x2);
%     plot(x_state(1,(N_loops_fb+1):N_loops_vf)+sigma_x',':b')
%     plot(x_state(1,(N_loops_fb+1):N_loops_vf)-sigma_x',':b')
    hold off
subplot(212),hold on
    plot(x_t_vec(2)*ones(n_loop,1),'--r')
    plot(x_state(2,(N_loops_fb+1):N_loops_vf),'k')
%     sigma_y2=zeros(n_loop,1);
%     sigma_y2(:)=P_cov(1,1,(N_loops_fb+1):N_loops_vf);
%     sigma_y=sqrt(sigma_x2);
%     plot(x_state(2,(N_loops_fb+1):N_loops_vf)+sigma_y',':b')
%     plot(x_state(2,(N_loops_fb+1):N_loops_vf)-sigma_y',':b')
    hold off
