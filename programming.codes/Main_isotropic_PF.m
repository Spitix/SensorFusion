...
%   Area parameters and number of iterations are global variables
    ...
    %   Geolocation process parameters
        ...
        %   EKF (UKF) initialisation:
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            Q=diag([((.01))^2 ((.01))^2]);                             %   Process noise matrix: better to be small std for position and power
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            %---------- < Q_KF must be set up appropriately > ------------%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            R=0.1^2;                                                	%   Specify noise on alpha: enable if wanted  
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            %---------- < Q_KF must be set up appropriately > ------------%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            P_cov_ini=diag([(3000)^(2) (3000)^(2)]);                            %   Initial state covariance guess - Change if needed
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
N                   =           1000 ;                                                               % Particles of State Estimation
a_x = x_state_ini(1) - sqrt(P_cov_ini(1,1));
b_x = x_state_ini(1) + sqrt(P_cov_ini(1,1));
a_y = x_state_ini(2) - sqrt(P_cov_ini(2,2));
b_y = x_state_ini(2) + sqrt(P_cov_ini(2,2));

x0_part = a_x + (b_x - a_x).*rand(N,1);
y0_part = a_y + (b_y - a_y).*rand(N,1);
...
%.. Initialize Particle Filter
    ...
    for i   =   1 : N
        ...
        y_part(i)       =           y0_part(i) ;                                        % Assign Particles of State
        
    end
figure(2),hold off, histogram2(x_part(:),y_part(:)) 
...
%             x_state(:,k) = x_t_vec;
            ...
            %%% and UKF
%             x_state(:,k)=KF_PF(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state_ini,P_cov_ini,F_KF,G_KF,Q_KF,R_KF);

            
            x_vec_all_ini = x_vec_all(1,:);
            
            for i   =   1 : N
            sqrt(Q(1,1)) * randn
            x_part_pr(i) =          x_part(i) + sqrt(Q(1,1)) * randn;
            y_part_pr(i) =          y_part(i) + sqrt(Q(2,2)) * randn;

            r_ini       =           sqrt((x_part_pr(i)-x_vec_all_ini(1))^2+(y_part_pr(i)-x_vec_all_ini(2))^2+h_0^2);
            r_act       =           sqrt((x_part_pr(i)-x_vec_all(k,1))^2+(y_part_pr(i)-x_vec_all(k,2))^2+h_0^2);
            alpha(i)    =           r_ini^2/r_act^2;    % Perform the Time Propagation Step to obtain a priori Particles
            
            v_hat       =           P_r_filt_ratio(k,1) - alpha(i)                                     % Error of Measurement
        ...
        y_hat_part      =           mean(y_part) ;
  
        x_state(:,k)         =           [x_hat_part,y_hat_part];
...
%             x_state(:,k) = x_t_vec;
            %%% students must uncomment the following line and design a EKF
            %%% and UKF
%             x_state(:,k)=KF_PF(x_vec_all(1,:),x_vec_all(k,:),h_0,P_r_filt_ratio(k,1),x_state(:,k-1),P_cov(:,:,k-1),F_KF,G_KF,Q_KF,R_KF);
       ...
       figure(2), histogram2(x_part(:),y_part(:)) 
        % The Particle Filter Estimate is the Mean of the Particles
        x_hat_part      =           mean(x_part) ;
        y_hat_part      =           mean(y_part) ;
